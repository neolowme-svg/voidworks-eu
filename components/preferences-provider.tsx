...
export function PreferencesProvider({ children }: { children:React.ReactNode }) {
  const [locale,setLocaleState]=useState<Locale>("nl"); const [theme,setThemeState]=useState<Theme>("dark"); const [consent,setConsentState]=useState<Consent>("unset");
  useEffect(()=>{
    ...
    if(storedConsent==="preferences"){
      const storedLocale=localStorage.getItem("voidworks-locale") as Locale|null; const storedTheme=localStorage.getItem("voidworks-theme") as Theme|null;
  ...
  useEffect(()=>{document.documentElement.lang=locale;document.documentElement.dataset.theme=theme;if(consent==="preferences"){localStorage.setItem("voidworks-locale",locale);localStorage.setItem("voidworks-theme",theme);}},[locale,theme,consent]);
  const setConsent=(value:Exclude<Consent,"unset">)=>{setConsentState(value);localStorage.setItem("voidworks-consent",value);if(value==="essential"){localStorage.removeItem("voidworks-locale");localStorage.removeItem("voidworks-theme");}};
