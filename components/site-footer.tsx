...
export function SiteFooter(){const{text}=usePreferences();return <footer className="site-footer"><div className="container footer-grid">
  <div className="footer-brand"><Image className="brand-dark" src="/assets/voidworks-wordmark.png" alt="Voidworks" width={460} height={150} unoptimized/><Image className="brand-light" src="/assets/voidworks-wordmark-light.png" alt="Voidworks" width={460} height={150} unoptimized/><p>{text.footer.text}</p></div>
  ...
  <div className="footer-column"><strong>{text.footer.account}</strong><Link href="/login">{text.nav.login}</Link><Link href="/register">{text.nav.register}</Link><Link href="/dashboard">{text.nav.dashboard}</Link><a href="https://discord.gg/SBtnUvrzg6" target="_blank" rel="noreferrer">{text.footer.discord} ↗</a></div>
