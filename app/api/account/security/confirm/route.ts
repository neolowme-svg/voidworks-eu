...
export async function POST(request:Request){
  ...
  try{const body=await request.json() as Record<string,unknown>;const action=String(body.action||"") as AccountAction;const code=String(body.code||"").trim();if(!allowed.has(action)||!/^\d{6}$/.test(code))return NextResponse.json({error:"INVALID_CODE"},{status:400});const supabase=await createClient();const{data:{user}}=await supabase.auth.getUser();if(!user||!(await validateAppSession(user.id)))return NextResponse.json({error:"NOT_AUTHENTICATED"},{status:401});if(!(await consumeRateLimit(`account-confirm:${user.id}:${action}`,10,900)))return NextResponse.json({error:"RATE_LIMIT"},{status:429});
    let fullName="",username="",password="",oldPassword="";const admin=createAdminClient();
    ...
    if(action==="change_password"){password=String(body.password||"");oldPassword=String(body.oldPassword||"");if(!oldPassword)return NextResponse.json({error:"CURRENT_PASSWORD_INVALID"},{status:400});if(password===oldPassword)return NextResponse.json({error:"SAME_PASSWORD"},{status:400});if(!strong(password))return NextResponse.json({error:"WEAK_PASSWORD"},{status:400});const email=String(user.email||"");const verify=await supabase.auth.signInWithPassword({email,password:oldPassword});if(verify.error||!verify.data.user)return NextResponse.json({error:"CURRENT_PASSWORD_INVALID"},{status:401});}
